package com.example.editphotovideo.library.zoomimg;
...
import com.example.editphotovideo.library.zoomimg.internal.MovementBounds;
import com.example.editphotovideo.library.zoomimg.internal.ZoomBounds;
import com.example.editphotovideo.library.zoomimg.utils.GravityUtils;


/**
 * Helper class that holds reference to {@link com.example.editphotovideo.library.zoomimg.Settings} object and controls some aspects of view
 * {@link com.example.editphotovideo.library.zoomimg.State}, such as movement bounds restrictions
 * (see {@link #getMovementArea(com.example.editphotovideo.library.zoomimg.State, RectF)}) and dynamic min / max zoom levels
 * (see {@link #getMinZoom(com.example.editphotovideo.library.zoomimg.State)} and {@link #getMaxZoom(com.example.editphotovideo.library.zoomimg.State)}).
 */
public class StateController {

    // Temporary objects
    private static final com.example.editphotovideo.library.zoomimg.State tmpState = new com.example.editphotovideo.library.zoomimg.State();
    private static final Rect tmpRect = new Rect();
    private static final RectF tmpRectF = new RectF();
    private static final PointF tmpPointF = new PointF();


    private final com.example.editphotovideo.library.zoomimg.Settings settings;
    ...
    StateController(com.example.editphotovideo.library.zoomimg.Settings settings) {
     ...
     * when {@link #updateState(com.example.editphotovideo.library.zoomimg.State)} method will be called.
     *
     * @param state State to be reset
     * @return {@code true} if reset was completed or {@code false} if reset is scheduled for future
     */
    boolean resetState(com.example.editphotovideo.library.zoomimg.State state) {
        isResetRequired = true;
        return updateState(state);
    }

    /**
     * Updates state (or resets state if reset was scheduled, see {@link #resetState(com.example.editphotovideo.library.zoomimg.State)}).
     *
     * @param state State to be updated
     * @return {@code true} if state was reset to initial state or {@code false} if state was
     * updated.
     */
    boolean updateState(com.example.editphotovideo.library.zoomimg.State state) {
        if (isResetRequired) {
    ...
    public void applyZoomPatch(@NonNull com.example.editphotovideo.library.zoomimg.State state) {
        if (zoomPatch > 0f) {
     ...
     */
    com.example.editphotovideo.library.zoomimg.State toggleMinMaxZoom(com.example.editphotovideo.library.zoomimg.State state, float pivotX, float pivotY) {
        ...
        final com.example.editphotovideo.library.zoomimg.State end = state.copy();
    ...
    @Nullable
    com.example.editphotovideo.library.zoomimg.State restrictStateBoundsCopy(com.example.editphotovideo.library.zoomimg.State state, com.example.editphotovideo.library.zoomimg.State prevState, float pivotX, float pivotY,
     ...
     */
    boolean restrictStateBounds(com.example.editphotovideo.library.zoomimg.State state, com.example.editphotovideo.library.zoomimg.State prevState, float pivotX, float pivotY,
                                boolean allowOverscroll, boolean allowOverzoom, boolean restrictRotation) {
        ...
        if (restrictRotation && settings.isRestrictRotation()) {
            ...
            if (!com.example.editphotovideo.library.zoomimg.State.equals(rotation, state.getRotation())) {
        ...
        if (!com.example.editphotovideo.library.zoomimg.State.equals(zoom, state.getZoom())) {
        ...
        if (!com.example.editphotovideo.library.zoomimg.State.equals(newX, state.getX()) || !com.example.editphotovideo.library.zoomimg.State.equals(newY, state.getY())) {
     ...
     */
    public float getMinZoom(@NonNull com.example.editphotovideo.library.zoomimg.State state) {
     ...
     * Note, that it may be different from {@link com.example.editphotovideo.library.zoomimg.Settings#getMaxZoom()}.
     */
    @SuppressWarnings({ "unused", "WeakerAccess" }) // Public API
    public float getMaxZoom(@NonNull com.example.editphotovideo.library.zoomimg.State state) {
     ...
     * {@link com.example.editphotovideo.library.zoomimg.Settings#getFitMethod()} is {@link com.example.editphotovideo.library.zoomimg.Settings.Fit#NONE}).
     */
    public float getFitZoom(@NonNull com.example.editphotovideo.library.zoomimg.State state) {
        return zoomBounds.set(state).getFitZoom();
    }

    /**
     * Calculates area in which {@link com.example.editphotovideo.library.zoomimg.State#getX()} &amp; {@link com.example.editphotovideo.library.zoomimg.State#getY()} values can change.
