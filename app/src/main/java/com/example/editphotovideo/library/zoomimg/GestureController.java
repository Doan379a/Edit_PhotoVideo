...
import com.example.editphotovideo.library.zoomimg.internal.ExitController;
import com.example.editphotovideo.library.zoomimg.internal.MovementBounds;
import com.example.editphotovideo.library.zoomimg.internal.detectors.RotationGestureDetector;
import com.example.editphotovideo.library.zoomimg.internal.detectors.ScaleGestureDetectorFixed;
...
/**
 * Handles touch events to update view's position state ({@link com.example.editphotovideo.library.zoomimg.State}) based on current
 ...
 * stuff.
 * <p>
 * State can be animated with {@link #animateStateTo(com.example.editphotovideo.library.zoomimg.State)} method.<br>
...
public class GestureController implements View.OnTouchListener {
    ...
    private final com.example.editphotovideo.library.zoomimg.State stateStart = new com.example.editphotovideo.library.zoomimg.State();
    private final com.example.editphotovideo.library.zoomimg.State stateEnd = new com.example.editphotovideo.library.zoomimg.State();

    private final View targetView;
    private final Settings settings;
    private final com.example.editphotovideo.library.zoomimg.State state = new com.example.editphotovideo.library.zoomimg.State();
    private final com.example.editphotovideo.library.zoomimg.State prevState = new com.example.editphotovideo.library.zoomimg.State();
     ...
     */
    @NonNull
    public com.example.editphotovideo.library.zoomimg.State getState() {
        return state;
    }
     ...
     * @see #animateStateTo(com.example.editphotovideo.library.zoomimg.State)
     */
    ...
    public boolean animateStateTo(@Nullable com.example.editphotovideo.library.zoomimg.State endState) {
        return animateStateTo(endState, true);
    }

    private boolean animateStateTo(@Nullable com.example.editphotovideo.library.zoomimg.State endState, boolean keepInBounds) {
        if (endState == null) {
            return false;
        }
        ...
        com.example.editphotovideo.library.zoomimg.State endStateRestricted = null;
        if (keepInBounds) {
    ...
    protected boolean onTouchInternal(@NonNull View view, @NonNull MotionEvent event) {
        ...
        if (isRestrictZoomRequested || isRestrictRotationRequested) {
            ...
            if (!exitController.isExitDetected()) {
                com.example.editphotovideo.library.zoomimg.State restrictedState = stateController.restrictStateBoundsCopy(
    ...
    protected boolean shouldDisallowInterceptTouch(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                final boolean isPannable = com.example.editphotovideo.library.zoomimg.State.compare(tmpRectF.width(), 0f) > 0
                        || com.example.editphotovideo.library.zoomimg.State.compare(tmpRectF.height(), 0f) > 0;
    ...
    protected boolean onFlingScroll(int dx, int dy) {
        ...
        return !com.example.editphotovideo.library.zoomimg.State.equals(prevX, toX) || !com.example.editphotovideo.library.zoomimg.State.equals(prevY, toY);
    }
    ...
    public interface OnStateChangeListener {
        void onStateChanged(com.example.editphotovideo.library.zoomimg.State state);

        void onStateReset(com.example.editphotovideo.library.zoomimg.State oldState, State newState);
    }
