...
class ViewPositionHolder implements ViewTreeObserver.OnPreDrawListener {
    ...
    private final com.example.editphotovideo.library.zoomimg.animation.ViewPosition pos = com.example.editphotovideo.library.zoomimg.animation.ViewPosition.newInstance();
    ...
    private void update() {
        if (view != null && listener != null && !isPaused) {
            boolean changed = com.example.editphotovideo.library.zoomimg.animation.ViewPosition.apply(pos, view);
            if (changed) {
