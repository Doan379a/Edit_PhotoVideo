package com.videomaker.photovideo.editvideo.utils.helper
...
class FileSaveHelper(private val mContentResolver: ContentResolver) : LifecycleObserver {
    ...
    fun createFile(fileNameToSave: String, folder: String?, listener: OnFileCreateResult?) {
        ...
        executor!!.submit {
            ...
            try {
                ...
                val editedImageUri =
                    getEditedImageUri(fileNameToSave, folder,newImageDetails, imageCollection)
    ...
    private fun getEditedImageUri(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && folder != null) {
            newImageDetails.put(MediaStore.Images.Media.RELATIVE_PATH, "DCIM/$folder")
        }
